...
(function () {
  ...
  let metaPixelInitialized = Boolean(window.__MBCT_META_PIXEL_INITIALIZED);

  function enableMetaPixel() {
    if (metaPixelInitialized || window['fbq-disabled']) return;
    const pixelId = new URLSearchParams(window.location.search).get('pixel_id') || CONFIG.pixelId;
    ...
    const initialize = () => {
      ...
      window.__MBCT_META_PIXEL_INITIALIZED = true;
    };
  ...
  function initCookieConsent() {
    ...
    if (consent === 'accepted') enableMetaPixel();
